...
const nameGreeting = nameCheck("Zalgo");
...
console.table(theEliminator(contestants, loser));
...
stringToUpperCase(sampleString);
...
console.log(emailCheck("email@google.com"));
...
//CODE HERE
function goldToFrogs(gold) {
  return Math.floor(gold / 3); // floor returns largest whole number
}
const totalFrogs = goldToFrogs(4);
...
//CODE HERE
function goldToFrogs2(gold) {
  return (gold / 3).toFixed(); // set floating point precision to default
}
const totalFrogs2 = goldToFrogs2(4);
...
function isAscending(array) {
  // array.reverse() // debug: reverse the array
  for (let index = 0; index < array.length; index++) {
    const element = array[index];
    if (element + 1 === array[index + 1]) {
      return true;
    } else {
      return false;
    }
